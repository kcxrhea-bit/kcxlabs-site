...
async function handler(request: Request): Promise<Response>{
 const method=requireMethod(request,"POST");if(method)return method;const context=await requireDevice(request);if(isResponse(context))return context;const body=await readJson(request);const id=requestUrl(request).pathname.split("/").at(-2)??"";const localPath=typeof body?.localPath==="string"?body.localPath.trim():"";const sizeBytes=typeof body?.sizeBytes==="number"?body.sizeBytes:NaN;const sha256=typeof body?.sha256==="string"?body.sha256.toLowerCase():"";if(!id||!localPath||body?.verified!==true)return json(400,{error:"verification_required"});
