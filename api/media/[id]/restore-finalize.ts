...
export default async function handler(request:Request):Promise<Response>{
 const method=requireMethod(request,"POST");if(method)return method;const context=await requireDevice(request);if(isResponse(context))return context;const id=new URL(request.url).pathname.split("/").at(-2)??"";if(!id)return json(400,{error:"invalid_request"});
