...
import { internalError, isResponse, json, readJson, requestUrl, requireDevice, toNodeHandler } from "../../_lib/http.js";
import { normalizeRetentionDays, type MediaVisibility } from "../../../../src/media/types.js";
...
async function handler(request: Request): Promise<Response> {
 const context=await requireDevice(request);if(isResponse(context))return context;const id=requestUrl(request).pathname.split("/").pop()??"";if(!id)return json(400,{error:"invalid_request"});const repo=mediaRepository(createDb(context.config.database));
 ...
 if(request.method==="DELETE"){return await repo.softDelete(context.ownerId,id)?new Response(null,{status:204}):json(404,{error:"not_found"});}
