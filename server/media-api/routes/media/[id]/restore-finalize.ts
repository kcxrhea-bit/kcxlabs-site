import { archiveRepository, createDb, mediaRepository } from "../../../_lib/db.js";
import { internalError, isResponse, json, requestUrl, requireDevice, requireMethod, toNodeHandler } from "../../../_lib/http.js";
import { headObject, r2Context } from "../../../_lib/r2.js";
async function handler(request:Request):Promise<Response>{
 const method=requireMethod(request,"POST");if(method)return method;const context=await requireDevice(request);if(isResponse(context))return context;const id=(requestUrl(request).pathname.split("/").slice(-2)[0]??"");if(!id)return json(400,{error:"invalid_request"});
