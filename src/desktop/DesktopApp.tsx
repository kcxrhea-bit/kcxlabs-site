...
import type { ActivityEntry, CatalogProject, DeploymentProgress, DeploymentReadiness, DeploymentResult, DesktopStatus, DiscoveredProject, NewCatalogProject, PreviewStatus, ThemeScan, WebsiteChangePreview, WebsiteChangeRequest, ReleaseStorageSettings } from "../shared/desktop";
...
/></>;
  ...
  if (page === "Settings") return <SettingsPageComplete />;
...
function SettingsPage() { const [settings,setSettings]=useState<ReleaseStorageSettings|null>(null); const [form,setForm]=useState({endpoint:"",bucket:"",region:"auto",publicBaseUrl:"",accessKeyId:"",secretAccessKey:""}); const [message,setMessage]=useState(""); useEffect(()=>{void window.kcxDesktop?.getReleaseStorageSettings().then((s)=>{setSettings(s);setForm((f)=>({...f,endpoint:s.endpoint,bucket:s.bucket,region:s.region,publicBaseUrl:s.publicBaseUrl}));});},[]); return <><PageIntro eyebrow="Settings" title="Local desktop settings">KCx Labs keeps these app settings locally on this computer. Website deployment and publication are always separate, explicit actions.</PageIntro><div className="desktop-summary-grid"><Panel title="Release storage"><p>Status: <strong>{settings?.configured?"CONFIGURED":"INCOMPLETE / UNAVAILABLE"}</strong></p><label>Endpoint<input value={form.endpoint} onChange={e=>setForm({...form,endpoint:e.target.value})}/></label><label>Bucket<input value={form.bucket} onChange={e=>setForm({...form,bucket:e.target.value})}/></label><label>Region<input value={form.region} onChange={e=>setForm({...form,region:e.target.value})}/></label><label>Public base URL<input value={form.publicBaseUrl} onChange={e=>setForm({...form,publicBaseUrl:e.target.value})}/></label><label>Access key ID<input value={form.accessKeyId} onChange={e=>setForm({...form,accessKeyId:e.target.value})}/></label><label>Secret access key<input type="password" value={form.secretAccessKey} onChange={e=>setForm({...form,secretAccessKey:e.target.value})} placeholder={settings?.hasSecretKey?"Configured; enter only to change":"Not configured"}/></label><button className="desktop-action" onClick={async()=>{try{const next=await window.kcxDesktop!.updateReleaseStorageSettings(form);setSettings(next);setForm({...form,accessKeyId:"",secretAccessKey:""});setMessage("Release storage settings saved.")}catch(e){setMessage(e instanceof Error?e.message:"Settings could not be saved.")}}}>Save release storage</button>{message&&<p>{message}</p>}</Panel><Panel title="Local storage"><p>Project catalog data, activity history, and preview state are stored in the desktop app’s local data folder.</p><Status>Local only</Status></Panel><Panel title="Deployment"><p>This desktop app can build and preview. It does not automatically deploy a website.</p><Status tone="warning">Manual action required</Status></Panel></div></>; }
function SettingsPageComplete() { const [settings,setSettings]=useState<ReleaseStorageSettings|null>(null); const [form,setForm]=useState({endpoint:"",bucket:"",region:"auto",publicBaseUrl:"",accessKeyId:"",secretAccessKey:""}); const [message,setMessage]=useState(""); const refresh=async()=>{const s=await window.kcxDesktop!.getReleaseStorageSettings();setSettings(s);setForm(f=>({...f,endpoint:s.endpoint,bucket:s.bucket,region:s.region,publicBaseUrl:s.publicBaseUrl}));}; useEffect(()=>{void refresh();},[]); const configured=Boolean(form.endpoint.trim()&&form.bucket.trim()&&form.region.trim()&&((settings?.hasAccessKey&&settings?.hasSecretKey)||(form.accessKeyId.trim()&&form.secretAccessKey.trim()))); return <><PageIntro eyebrow="Settings" title="Local desktop settings">Release storage is local-only. Credentials remain in the Electron main process.</PageIntro><div className="desktop-summary-grid"><Panel title="Release storage"><p>Configuration: <strong>{settings?.configured?"CONFIGURED":"UNCONFIGURED"}</strong></p><p>Connection: {settings?.lastConnectionTestSucceeded===true?"PASS":settings?.lastConnectionTestSucceeded===false?"FAIL":"NOT TESTED"}</p><p>Certification: <strong>{settings?.certificationState??"NOT CERTIFIED"}</strong></p><p>Public download: {settings?.lastPublicDownloadResult??(settings?.publicBaseUrl?"NOT TESTED":"NOT CONFIGURED")}</p><label>Endpoint<input value={form.endpoint} onChange={e=>setForm({...form,endpoint:e.target.value})}/></label><label>Bucket<input value={form.bucket} onChange={e=>setForm({...form,bucket:e.target.value})}/></label><label>Region<input value={form.region} onChange={e=>setForm({...form,region:e.target.value})}/></label><label>Public base URL<input value={form.publicBaseUrl} onChange={e=>setForm({...form,publicBaseUrl:e.target.value})}/></label><label>Access key ID<input value={form.accessKeyId} onChange={e=>setForm({...form,accessKeyId:e.target.value})}/></label><label>Secret access key<input type="password" value={form.secretAccessKey} onChange={e=>setForm({...form,secretAccessKey:e.target.value})} placeholder={settings?.hasSecretKey?"Configured; enter only to change":"Not configured"}/></label><div className="desktop-action-row"><button className="desktop-action" disabled={!configured} onClick={async()=>{try{setSettings(await window.kcxDesktop!.updateReleaseStorageSettings(form));setForm(f=>({...f,accessKeyId:"",secretAccessKey:""}));setMessage("Saved");await refresh();}catch(e){setMessage(e instanceof Error?e.message:"Save failed")}}}>Save Settings</button><button className="desktop-action desktop-action-secondary" disabled={!settings?.configured} onClick={async()=>{setMessage("Testing…");const r=await window.kcxDesktop!.testReleaseStorage();setMessage(r.success?"PASS":`FAIL: ${r.message}`);await refresh();}}>Test Connection</button><button className="desktop-action desktop-action-secondary" disabled={!settings?.configured} onClick={async()=>{setMessage("Running probe…");const r=await window.kcxDesktop!.probeReleaseStorage();setMessage(r.errorCode==="BUSY"?"BUSY":r.success?"PASS":`FAIL: ${r.message}`);await refresh();}}>Run Provider Probe</button></div>{message&&<p>{message}</p>}</Panel><Panel title="Local storage"><p>Settings and certification metadata are stored locally. Secrets are OS-encrypted and never returned to the renderer.</p><Status>Local only</Status></Panel></div></>; }
  ...
  const sections: Array<[string, ReactNode]> = [
    ...
    [
      ...
      <>
        ...
        <p>Capability inspection checks the selected project for multiple project types and independently resolves each distribution target. READY means the target can be built now. PREPARABLE means KCx Labs can preview a safe setup path, such as adding electron-builder when Electron has no packager. NEEDS INPUT means a human choice is required. UNSUPPORTED means no supported backend was detected.</p>
    ...
    [
      "Build & package",
      <>
        <p><button className="desktop-link" onClick={() => navigate("Artifacts")}>Artifacts</button> inspects the registered project without changing it, detects multiple project types, and resolves Web, EXE, Windows installer, APK, ZIP, and Source through project-owned scripts or packaging backends. Use Preview setup before any dependency, script, or configuration change; nothing is installed or deployed during inspection.</p>
    ...
    [
      "Artifacts & staging",
      <>
        <p>STAGED means KCx Labs prepared a local artifact for the workflow. A staged artifact is not yet a public release and does not update production. If an older app left a matching staged file recorded as VALIDATED, use Recover staged state; it only succeeds after canonical-path, size, and SHA-256 checks.</p>
    ...
    [
      ...
      <>
        ...
        <p>Publishing requires explicit confirmation in the workflow. It copies the validated artifact into the public release area, creates a backup when replacing an existing file, and records release/catalog metadata. A previously published artifact with a legacy remote path may expose Reconcile published path; that action verifies the old and new provider objects plus public retrieval before updating local metadata and never deletes the old object.</p>
